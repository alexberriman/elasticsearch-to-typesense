...
describe("suggestTransformHints", () => {
  ...
  it("should generate hints for missing fields", () => {
    ...
    expect(hints).toContain("No Typesense field for Elasticsearch field \"price\"");
    expect(hints).toContain("No Typesense field for Elasticsearch field \"location\"");
  });
  ...
  it("should use custom match strategy when provided", () => {
    const customMatchStrategy = (elasticField: string, typesenseField: string) => {
    ...
    const hints = suggestTransformHints(customElasticSchema, customTypesenseSchema, customMatchStrategy);
  ...
  it("should handle empty schemas", () => {
    ...
    const hints = suggestTransformHints(emptyElasticSchema, emptyTypesenseSchema);
  ...
  it("should not generate hints for matching fields with compatible types", () => {
    ...
    expect(hints).not.toContain("No Typesense field for Elasticsearch field \"id\"");
    expect(hints).not.toContain("No Typesense field for Elasticsearch field \"name\"");
    expect(hints).not.toContain("No Typesense field for Elasticsearch field \"counter\"");
  });
});