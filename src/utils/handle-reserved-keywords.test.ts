...
import { isReservedKeyword, resolveReservedKeyword } from "./handle-reserved-keywords";
...
describe("resolveReservedKeyword", () => {
  ...
  it("should return original value for non-reserved keywords", () => {
    expect(resolveReservedKeyword("any_field", "not_now", mockSchema)).toBe("not_now");
  ...
  it("should return original value when field is not in schema", () => {
    expect(resolveReservedKeyword("missing_field", "now", mockSchema)).toBe("now");
  });

  it("should resolve 'now' to timestamp for int fields", () => {
    const timestamp = Date.now(); // 1672531200000
    expect(resolveReservedKeyword("int_field", "now", mockSchema)).toBe(timestamp);
    expect(resolveReservedKeyword("int64_field", "now", mockSchema)).toBe(timestamp);
  });

  it("should resolve 'now' to float timestamp for float fields", () => {
    const timestamp = Date.now() * 1.0; // 1672531200000.0
    expect(resolveReservedKeyword("float_field", "now", mockSchema)).toBe(timestamp);
  });

  it("should resolve 'now' to ISO string for other field types", () => {
    const isoString = new Date().toISOString(); // "2023-01-01T00:00:00.000Z"
    expect(resolveReservedKeyword("string_field", "now", mockSchema)).toBe(isoString);
    expect(resolveReservedKeyword("bool_field", "now", mockSchema)).toBe(isoString);
  });
});