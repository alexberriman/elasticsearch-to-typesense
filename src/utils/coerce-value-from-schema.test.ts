...
describe("coerceValueFromSchema", () => {
  ...
  it("should return original value when field is not in schema", () => {
    ...
    expect(coerceValueFromSchema("missing_field", "test", mockSchema)).toBe("test");
  });
  ...
  it("should convert string values to floats for float fields", () => {
    expect(coerceValueFromSchema("float_field", "123.45", mockSchema)).toBe(123.45);
    expect(coerceValueFromSchema("float_field", 123.45, mockSchema)).toBe(123.45);
  });
  ...
  it("should convert values to booleans for bool fields", () => {
    ...
    expect(coerceValueFromSchema("bool_field", "TRUE", mockSchema)).toBe(true);
    expect(coerceValueFromSchema("bool_field", "false", mockSchema)).toBe(false);
    expect(coerceValueFromSchema("bool_field", true, mockSchema)).toBe(true);
  ...
  it("should convert values to strings for string fields", () => {
    ...
    expect(coerceValueFromSchema("string_field", true, mockSchema)).toBe("true");
    expect(coerceValueFromSchema("string_field", "test", mockSchema)).toBe("test");
  });

  it("should convert values to strings for geopoint fields", () => {
    expect(coerceValueFromSchema("geo_field", "47.1234,9.5678", mockSchema)).toBe("47.1234,9.5678");
