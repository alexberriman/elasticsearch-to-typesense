...
describe("resolveNestedField", () => {
  ...
  it("should resolve nested fields with dot notation", () => {
    expect(resolveNestedField("metadata.createdAt", testDocument)).toBe("2023-01-01");
    expect(resolveNestedField("metadata.author.name", testDocument)).toBe("John Doe");
    expect(resolveNestedField("metadata.author.contact.email", testDocument)).toBe("john@example.com");
  ...
  it("should return undefined for non-existent fields", () => {
    ...
    expect(resolveNestedField("metadata.nonexistent", testDocument)).toBeUndefined();
    expect(resolveNestedField("metadata.author.nonexistent", testDocument)).toBeUndefined();
    expect(resolveNestedField("metadata.author.contact.nonexistent", testDocument)).toBeUndefined();
  });

  it("should handle arrays in path", () => {
    expect(resolveNestedField("metadata.tags.0", testDocument)).toBe("test");
    expect(resolveNestedField("metadata.tags.1", testDocument)).toBe("document");
  ...
  it("should handle empty path segments", () => {
    ...
    expect(resolveNestedField("metadata..author.name", testDocument)).toBeUndefined();
  });

  it("should handle undefined document", () => {
    expect(resolveNestedField("id", undefined as any)).toBeUndefined();
    expect(resolveNestedField("metadata.author.name", undefined as any)).toBeUndefined();
  });

  it("should handle null document", () => {
    expect(resolveNestedField("id", null as any)).toBeUndefined();
    expect(resolveNestedField("metadata.author.name", null as any)).toBeUndefined();
  });
});