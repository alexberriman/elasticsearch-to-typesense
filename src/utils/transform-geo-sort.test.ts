...
describe("transformGeoSort", () => {
  ...
  it("should warn about invalid order values", () => {
    ...
    expect(result.warnings).toContain("Invalid sort order: invalid. Using 'asc' instead.");
  });
  ...
  it("should warn when no fields with coordinates are found", () => {
    ...
    expect(result.warnings).toContain("Invalid geo_distance sort: expected exactly one field with coordinates");
  });

  it("should warn when multiple fields with coordinates are found", () => {
    const result = transformGeoSort({
      "location1": { lat: 40.7128, lon: -74.0060 },
      "location2": { lat: 41.7128, lon: -75.0060 },
      "order": "asc"
    }, mockContext);
  ...
  it("should warn when coordinates are missing or invalid", () => {
    const result = transformGeoSort({
      "location": { lat: "invalid", lon: -74.0060 },
      "order": "asc"
    }, mockContext);
    
    expect(result.sort).toBeUndefined();
    expect(result.warnings).toContain("Invalid geo_distance coordinates for field 'location'");
  });
